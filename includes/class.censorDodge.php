...
class censorDodge {
    ...
    public function curlRequest($URL, $getParameters = array(), $postParameters = array()) {
        ...
        curl_setopt($curl, CURLOPT_HEADERFUNCTION, (function ($curl, $hl) use (&$body, $URL, &$headersAdded) {
            ...
            $hn = trim(strtolower(explode(":",$hl,2)[0])); if (in_array($hn, $allowedHeaders) && $curl) { $headersAdded[$hn] = true; header($hl); } //Control which headers are set as we receive them from cURL
            return strlen($hl);
        }));
